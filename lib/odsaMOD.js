...
(function ($) {
  ...
  // Send Single attempt to DB for storage and scoring
  const sendScoreData = (scoreData) => {
    jQuery.ajax({
      url: "/odsa_exercise_attempts/pe",
      type: "POST",
      data: JSON.stringify(scoreData),
  ...
  $(document).ready(function () {
    ...
    eventer(messageEvent, function (e) {
      ...
      if (data) {
        ...
        if (data.type === 'ka-attempt-saved') {
          // Re-emit as a app-level event
          var success = data.status === 'success';
          if (success) {
            // Update any local UI
          }
          if (window.ODSA && window.ODSA.MOD && window.ODSA.MOD.emit) {
            window.ODSA.MOD.emit('attemptSaved', [data]); // jQuery event args style
          }
          return;
        }
        if (data.subject === 'SPLICE.getState') {
          console.log("GET STATE RECIVED DATA: ", data);
          jQuery.ajax({
            url: "/odsa_user_interactions/latest_state/",
            type: "GET",
            data: {
            ...
            success: function (res) {
              ...
              $iframe = $(`#${data.exercise_name}_iframe`)
        ...
        if (data.subject === 'SPLICE.reportScoreAndState') {
          ...
          sendScoreData(data.scoreData);

          const message = {
            type: "SPLICE.reportScoreAndState",
            desc: {
              location: data.scoreData.exercise,
              score: data.score,
        ...
        if (data.subject === 'SPLICE.sendEvent') {
          ...
          data.type = "SPLICE.sendEvent";
          data.desc = structuredClone(data);
          window.ODSA.UTILS.logEvent(data);
          return;
        }
  ...
  // Simple event bus
  var _bus = $({});
  odsaMod.on = function (evt, handler) { _bus.on(evt, handler); };
  odsaMod.off = function (evt, handler) { _bus.off(evt, handler); };
  odsaMod.emit = function (evt, data) { _bus.trigger(evt, data); };
