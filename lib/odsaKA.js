...
(function () {
  ...
  // Fire-and-notify: does not return a promise, listen for postMessage in parent
  Khan.saveAttemptToServer = function (url, attemptData) {
    ...
    if (typeof SPLICE !== 'undefined' && SPLICE.reportScoreAndState && SPLICE.sendEvent) {
      var exerciseName = attemptData.problem_type ? attemptData.problem_type : 'unknown';
      SPLICE.reportScoreAndState(exerciseName, attemptData.complete, attemptData);
      SPLICE.sendEvent(exerciseName, 'attemptData', attemptData);
    } else {
        console.warn("SPLICE is not defined in this exercise.");
    }
    ...
    eventer(messageEvent, (event) => {
      ...
      if (data.target !== exerciseName) { return; }
      ...
      if (data.result === 'success') {
        ...
        if (data.responseData) {
          current_score = data.responseData.exercise_progress.current_score;
          highest_score = data.responseData.exercise_progress.highest_score;
          threshold = data.responseData.threshold;
        }
      ...
      if (data.result === 'failure') {
        ...
        var logMessage = "[" + (+new Date()) + "] request to " +
          url + " failed (" + data.status + ", " + data.statusText + ") " +
    ...
    promise.done(function (data) {
      ...
      // notify parent (module) that the attempt save succeeded
      try {
        parent.postMessage(JSON.stringify({
          type: 'ka-attempt-saved',
          status: 'success',
        ...
        // no-op; keep UI consistent even if postMessage target isn't available
      }
    });
    ...
    promise.fail(function (xhr) {
      ...
      // Keep existing failure notice for existing listeners
      parent.postMessage('{"exercise":"' + Khan.exerciseName + '","proficient":' + false + ',"error":' + true + '}', MODULE_ORIGIN);
