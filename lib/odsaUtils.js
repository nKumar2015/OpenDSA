  ...
  */
  ...
  $("body").on("jsav-log-event", function (event, eventData) {
      ...
      if (eventData.type !== 'jsav-exercise-grade' && 
          ...
          window.SPLICE.sendEvent(ODSA.SETTINGS.AV_NAME, eventData.type, eventData);
          return;
      }
      ...
      if (window.SPLICE.reportScoreAndState) {
          ...
          const av = ODSA.SETTINGS.AV_NAME;
          ...
          const scoreData = {
            ...
            module:      getModuleID(),
            score:       score,
            steps_fixed: eventData.currentStep,
            tstamp:      (new Date(eventData.tstamp)).getTime(),
            total_time:  window.TimeMe.getTimeOnPageInMilliseconds(av),
          ...
          if (hasBook()) {
            ...
            if (isFullModule()) {
              scoreData['inst_chapter_module_id'] = getChapterModuleID();
            } else {
              scoreData['inst_section_id'] = window.ODSA.TP.instSectionId;
            }
          } else if (isStandaloneModule()) {
            scoreData['inst_module_version_id'] = getInstModuleVersionId();
          } else {
            scoreData['inst_course_offering_exercise_id'] = getInstCourseOfferingExerciseId();
          }

          scoreData['inst_book_section_exercise_id'] = window.PARAMS.instBookSectionExerciseId

          window.SPLICE.reportScoreAndState(eventData.av, score, scoreData, eventData);
      }
  });
